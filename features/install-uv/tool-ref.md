...
The Unix shell installer is generated by `cargo-dist` and performs these steps:

1. **Configuration from environment**: reads `UV_DOWNLOAD_URL` / `INSTALLER_DOWNLOAD_URL` / `UV_INSTALLER_GHE_BASE_URL` / `UV_INSTALLER_GITHUB_BASE_URL` to build `ARTIFACT_DOWNLOAD_URLS`. Default uses Astral CDN (`releases.astral.sh`) first, then `github.com` as fallback. `UV_NO_MODIFY_PATH`, `UV_DISABLE_UPDATE`, `UV_UNMANAGED_INSTALL`, and `UV_GITHUB_TOKEN` are also read.
2. **Architecture detection** (`get_architecture`): runs `uname -s` and `uname -m`. Linux libc detected via `ldd --version 2>&1 | grep -q musl` (musl vs gnu assumed). macOS Rosetta 2 corrected via `sysctl hw.optional.arm64`. 32-bit userland on 64-bit kernel detected by reading ELF bitness from `/proc/self/exe` header bytes. armv7 without NEON/SIMD (checked via `/proc/cpuinfo Features`) falls back to `arm`. illumos detected via `/usr/bin/uname -o`. Handles CYGWIN/MSYS/MINGW for Windows POSIX shells. CPU aliases resolved: `armv6l` → `arm`, `armv7l`/`armv8l` → `armv7`, `aarch64`/`arm64` → `aarch64`, `x86_64`/`amd64`/`x64` → `x86_64`, `ppc64le` → `powerpc64le`.
3. **Archive selection** (`select_archive_for_arch`): maps detected arch+libc to the best artifact via a `case` statement. For glibc targets, checks minimum glibc version (`check_glibc` using `ldd --version`) and falls back to the musl static artifact when too old. Fallback chains: `aarch64-linux-gnu` (needs glibc ≥2.28) → `aarch64-linux-musl`; `x86_64-linux-gnu` (≥2.17) → `x86_64-linux-musl`; `armv7-linux-gnu` (≥2.17) → `armv7-linux-musl`; `riscv64-linux-gnu` (≥2.31) → `riscv64-linux-musl`. No glibc fallback for `powerpc64le` or `s390x` (glibc-only; no musl artifact). armv6l always uses `arm-unknown-linux-musleabihf`.
4. **Hardcoded checksums**: each supported artifact has an embedded SHA256 hash. After download, integrity is verified via `sha256sum`/`shasum`; verification is skipped with a warning if no tool is available (soft-fail).
5. **Multi-URL download**: iterates through `ARTIFACT_DOWNLOAD_URLS` and tries each base URL in order until a download succeeds.
6. **Updater binary**: if an `uv-update` artifact exists and `INSTALL_UPDATER=1`, downloads and installs the self-updater binary alongside `uv`/`uvx`.
7. **Install receipt**: writes `$XDG_CONFIG_HOME/uv/uv-receipt.json` (or `$LOCALAPPDATA/uv/` on Windows POSIX shells) containing install prefix, binary list, version, and provider metadata. This receipt is required for `uv self update` to work; its absence causes `uv self update` to refuse to run.
8. **PATH modification**: when `NO_MODIFY_PATH=0` (default), writes `env`/`env.fish` helper scripts to the install directory and adds source lines to detected shell profile files (`~/.profile`, `~/.bashrc`, `~/.bash_profile`, `~/.bash_login`, zsh startup files, fish `conf.d/`). Also writes to `$GITHUB_PATH` in GitHub Actions contexts. Skipped if install dir is already on PATH.
9. **HOME fallback**: uses `getent passwd $(id -un)` when `$HOME` is unset, for compatibility with minimal Linux environments.
10. **`UV_UNMANAGED_INSTALL` mode**: installs to the given fixed path, disables PATH modification, and disables receipt and updater installation — intended for scripted/ephemeral environments.
