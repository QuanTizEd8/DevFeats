...
**Implementation design notes** (for Phase 3/4, not research findings): The nvm method will use `github__latest_tag nvm-sh/nvm` to resolve the latest nvm release tag. The binary method will use `verify__sha` (with a hash extracted from `SHASUMS256.txt`) for SHA-256 verification; for multi-user PATH configuration, `users__resolve_list` is called with `ADD_USERS="$USERS"`, then `shell__system_path_files` / `shell__user_path_files` + `shell__sync_block` from `lib/shell.bash` are used to write PATH and nvm init snippets to shell startup files. There is no `shell__export_path` function in `lib/shell.bash`; the correct pattern is `shell__system_path_files` → `shell__sync_block` (as used in `install-miniforge`). These are design recommendations, not installation research.
...
- [lib/github.bash](../../../lib/github.bash) — `github__latest_tag` function for resolving latest nvm release tag.
- [lib/verify.bash](../../../lib/verify.bash) — `verify__sha` for verifying downloaded binaries against SHASUMS256.txt.
- [lib/shell.bash](../../../lib/shell.bash) — `shell__system_path_files`, `shell__user_path_files`, `shell__sync_block` for PATH and shell initialisation configuration.
- [lib/users.bash](../../../lib/users.bash) — `users__resolve_list` for multi-user targeting.
