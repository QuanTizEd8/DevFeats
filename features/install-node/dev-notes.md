...
- **Spec:**
  ...
  - **`method=nvm` — `current` symlink:** nvm creates and maintains `$NVM_DIR/current -> $NVM_DIR/versions/node/$_NODE_VERSION` automatically when `NVM_SYMLINK_CURRENT=true` is exported before install.
  ...
  - **`method=nvm` — per-binary symlinks (root):** for each binary in `node npm npx corepack`, if `$NVM_DIR/current/bin/<binary>` exists, create `ln -sf "$NVM_DIR/current/bin/<binary>" /usr/local/bin/<binary>`. This ensures command discovery in non-interactive contexts that do not source shell startup files.
...
- **Spec:**
  ...
  - **Per-user writes (both methods):** After system-wide writes, if `_USERS_ARR` is non-empty (resolved earlier in the script), for each user obtain their home directory via `users__resolve_home "$u"`. Per-user writes always run regardless of whether `EXPORT_PATH` is `"auto"` or an explicit file list (as long as `EXPORT_PATH ≠ ""`). Then:
