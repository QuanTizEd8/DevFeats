...
class InstallScriptGenerator:
    ...
    def generate(self, feature_id: str, metadata: dict) -> dict[Path, str]:
        ...
        * Defaults:        empty array  ()
        ...
        header = self._generate_block(feature_name, options, dependencies)
        full_script = self._shfmt_format(f"{header}\n\n{body}\n")
        return {target_file: full_script}

    def _generate_block(self, feature_name: str, options: dict, dependencies: dict | None = None) -> str:
    ...
    def _section_arg_parse(self, options: dict) -> str:
        ...
        for key, opt in options.items():
            ...
            if typ == "array":
                case_arms.append(self._render_template("case_arm_array", FLAG=flag, VAR=vname, KEY=key))
                env_reads.append(self._render_template("env_read_array", VAR=vname, KEY=key))
            else:
                case_arms.append(self._render_template("case_arm_scalar", FLAG=flag, VAR=vname, KEY=key))
                env_reads.append(self._render_template("env_read_scalar", VAR=vname, KEY=key))
        ...
        for key, opt in options.items():
            ...
            if typ == "array":
                ...
                if raw_default == "" or raw_default is None:
                    blocks.append(self._render_template("default_array_empty", VAR=vname, KEY=key))
                else:
                    ...
                    blocks.append(self._render_template("default_array_value", VAR=vname, ESCAPED=escaped, KEY=key, DISP=disp))
            else:
                ...
                blocks.append(self._render_template("default_scalar", VAR=vname, RHS=rhs, KEY=key, DISP=disp))
        return "\n".join(blocks)
    ...
    def _section_validation(self, options: dict) -> str:
        ...
        if enum_opts:
            ...
            for key, opt in enum_opts:
                ...
                validations.append(self._render_template(tpl, VAR=vname, KEY=key, PATTERN=pattern, EXPECTED=expected))
    ...
    def _section_dep_helpers(self, run_deps: dict, build_deps: dict) -> str:
        ...
        for group_name in run_deps:
            blocks.append(self._render_template("dep_helper_run", SAFE=group_name.replace("-", "_"), GROUP=group_name))
        for group_name in build_deps:
            blocks.append(self._render_template("dep_helper_build", SAFE=group_name.replace("-", "_"), GROUP=group_name))
        return "\n".join(blocks)
