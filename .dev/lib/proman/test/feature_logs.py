...
from .names import FeatureTestRun, bind_mount_container_log_path
...
def copy_log_to_bind_mount_fragment(
    ...
    *,
    ...
    """Shell fragment: copy install log onto the host bind-mount log path."""
    dest = bind_mount_container_log_path(run)
    qdest = dest.replace("'", "'\\''")
    if log_path:
        ...
        return (
            f'if [ -f "{qpath}" ]; then '
            "mkdir -p /log-out && "
            f'cp "{qpath}" "{qdest}" 2>/dev/null || true; '
            "fi"
        )
    return (
        'if [ -n "${LOG_FILE:-}" ] && [ -f "${LOG_FILE}" ]; then '
        "mkdir -p /log-out && "
        f'cp "${{LOG_FILE}}" "{qdest}" 2>/dev/null || true; '
        "fi"
    )
...
def append_bind_mount_copy_to_test_script(
    ...
    *,
    ...
    fragment = copy_log_to_bind_mount_fragment(run, log_path=log_path)
