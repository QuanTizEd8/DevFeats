...
def project_slug(owner: str, repo: str) -> str:
    """Return ``<owner>-<repo>`` (legacy slug form used in test-runner paths)."""
    return f"{owner}-{repo}"
...
def feat_share_dir(feature_id: str, owner: str, repo: str) -> str:
    ...
    This is the single authoritative definition used when generating the
    bash header (``_FEAT_SHARE_DIR`` variable) and test-runner injection.
...
def export_profile_d(feature_id: str, owner: str, repo: str) -> str:
    ...
    This is the single authoritative definition used when generating the
    bash header (``_EXPORT_PROFILE_D`` variable) and test-runner injection.
