...
# POSIX logging API — shared by install.sh and logging.bash.
#
# Pre-setup records append to _LOGGING__PENDING_FILE (buffer-only, no live stderr).
# When lib/logging.bash is loaded it sets _LOGGING__BASH_BACKEND=1 and provides
...
# With logging.bash loaded, logging__set_fn_prefix 1 / logging__setup --fn-prefix prepends
