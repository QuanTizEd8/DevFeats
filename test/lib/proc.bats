...
@test "proc__run_command_form runs JSON string via sh -c" {
  run bash -c 'source "$1" && printf %s "\"echo hello\"" | proc__run_command_form' _ "${LIB_ROOT}/proc.sh"
  assert_output "hello"
  assert_success
}

@test "proc__run_command_form runs JSON array argv" {
  run bash -c 'source "$1" && printf %s "[\"/bin/sh\",\"-c\",\"echo arr\"]" | proc__run_command_form' _ "${LIB_ROOT}/proc.sh"
  assert_output "arr"
  assert_success
}
...
@test "proc__run_command_form object form runs keyed commands" {
  run bash -c 'source "$1" && printf %s "{\"one\":\"echo A\",\"two\":\"echo B\"}" | proc__run_command_form' _ "${LIB_ROOT}/proc.sh"
  assert_success
...
@test "proc__run_command_form --cwd changes working directory for string form" {
  ...
  run bash -c 'source "$1" && printf %s "\"pwd\"" | proc__run_command_form --cwd "$2"' _ "${LIB_ROOT}/proc.sh" "$_tmp"
  assert_success
