...
@test "proc__run_command_form runs JSON string via sh -c" {
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "\"echo hello\"" | proc__run_command_form' _ "${LIB_ROOT}"
  assert_output "hello"
  assert_success
}

@test "proc__run_command_form runs JSON array argv" {
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "[\"/bin/sh\",\"-c\",\"echo arr\"]" | proc__run_command_form' _ "${LIB_ROOT}"
  assert_output "arr"
  assert_success
}
...
@test "proc__run_command_form object form runs keyed commands" {
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "{\"one\":\"echo A\",\"two\":\"echo B\"}" | proc__run_command_form' _ "${LIB_ROOT}"
  assert_success
...
@test "proc__run_command_form --cwd changes working directory for string form" {
  ...
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "\"pwd\"" | proc__run_command_form --cwd "$2"' _ "${LIB_ROOT}" "$_tmp"
  assert_success
...
@test "proc__run_command_form --cwd changes working directory for object form" {
  ...
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "{\"probe\":\"pwd\"}" | proc__run_command_form --cwd "$2"' _ "${LIB_ROOT}" "$_tmp"
  assert_success
  # macOS symlinks /tmp → /private/tmp; compare realpaths.
  _got="$(cd "$output" && pwd -P)"
  _want="$(cd "$_tmp" && pwd -P)"
...
@test "proc__run_command_form fails on unsupported JSON types" {
  run bash -c 'source "$1/file.sh" && source "$1/logging-api.sh" && source "$1/logging.sh" && source "$1/bootstrap.sh" && source "$1/json.sh" && source "$1/proc.sh" && printf %s "42" | proc__run_command_form' _ "${LIB_ROOT}"
  assert_failure
}
