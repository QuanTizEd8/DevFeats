...
# Fonts are installed under devfeats-install-fonts-*/gh/JetBrains/JetBrainsMono/<tag>/<id>/.
set -e
...
check "font files installed from gh" bash -c 'find '"$_FONTS"' -path "*/devfeats-install-fonts-*/gh/JetBrains/JetBrainsMono/*" -name "*.ttf" | grep -q .'
