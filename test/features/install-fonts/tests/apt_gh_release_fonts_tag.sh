...
# devfeats-install-fonts-*/gh/<owner>/<repo>/<tag>/<id>/.
set -e
...
check "font files installed from gh tag" bash -c 'find '"$_FONTS"' -path "*/devfeats-install-fonts-*/gh/JetBrains/JetBrainsMono/*" -name "*.ttf" | grep -q .'
