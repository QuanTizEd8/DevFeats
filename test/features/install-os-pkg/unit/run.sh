...
#   bash run.sh [<repo-root>]
#
# The first positional argument sets the repo root (passed by the test runner
# so this script works inside containers where git is not available).
# Defaults to walking up from $0 — only reliable on the host.
#
...
INSTALL_SH="$REPO_ROOT/src/install-os-pkg/install.bash"
