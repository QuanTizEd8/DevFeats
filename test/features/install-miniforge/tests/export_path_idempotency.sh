...
check "profile.d block updated to /opt/myforge/bin" grep -q '/opt/myforge/bin' "/etc/profile.d/${_EXPORT_PROFILE_D}"
...
check "profile.d block has no old /opt/conda/bin" bash -c '! grep -q "/opt/conda/bin" "/etc/profile.d/${_EXPORT_PROFILE_D}"'
...
check "profile.d has exactly one begin marker" bash -c '[ "$(grep -c ">>> conda PATH (install-miniforge) >>>" "/etc/profile.d/${_EXPORT_PROFILE_D}")" -eq 1 ]'
