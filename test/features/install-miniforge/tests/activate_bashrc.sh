...
check "miniforge end marker in /etc/bash.bashrc" grep -Fq "# <<< prefix activation (install-miniforge) <<<" /etc/bash.bashrc
...
check "miniforge begin marker not duplicated" bash -c '[ "$(grep -Fc "# >>> prefix activation (install-miniforge) >>>" /etc/bash.bashrc)" -eq 1 ]'
