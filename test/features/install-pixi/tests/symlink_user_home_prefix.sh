...
check "/root/.local/bin/pixi symlink exists" test -L /root/.local/bin/pixi
check "symlink target is /root/pixi/bin/pixi" bash -c '[ "$(readlink /root/.local/bin/pixi)" = "/root/pixi/bin/pixi" ]'
check "pixi callable via symlink" /root/.local/bin/pixi --version
