...
1. Validate `metadata.yaml` files against `metadata.schema.json`.
"""
...
def read_metadata(feature_id: str) -> dict | None:
    ...
    try:
        data = yaml.safe_load(metadata_filepath.read_text())
...
def _get_metadata_schema_validator():
    metadata_schema = json.loads(METADATA_SCHEMA_FILEPATH.read_text())
