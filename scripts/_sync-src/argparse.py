...
def generate_block(feature_name: str, options: dict, dependencies: dict | None = None) -> str:
    ...
    If ``_dependencies.build`` is non-empty, ``_all_build_deps__cleanup`` is
    ...
    # has_ospkg is true when the metadata has a `_dependencies` key (even empty),
    ...
    if run_deps and "base" in run_deps:
        lines.append("# Install dependencies/run/base.yaml when root (see argparse.py).")
        lines.append('if [ "$(id -u)" -eq 0 ]; then')
