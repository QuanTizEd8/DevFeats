...
- `./`: Root-level files.
  - `install.sh`: POSIX bootstrap for the standalone installer — finds bash ≥4, fetches `install.bash` from the repo, forwards args (see file header for usage).
  - `install.bash`: Bash ≥4 standalone installer — feature or manifest mode; downloads `lib/` helpers from the repo (see file header for usage). These entrypoints stay at the repo root (not under `dist/`).
