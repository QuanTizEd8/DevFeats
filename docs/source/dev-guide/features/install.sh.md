...
`install.sh` at the feature root is generated from `features/install.sh` by
...
- `features/install.sh` is a POSIX `sh` script. Its only job is to ensure `bash` is
  present and then `exec bash install.bash "$@"`.
- `install.bash` is written in bash ≥4 and uses all library features
  freely.

`features/install.sh` knows how to install bash on every supported distro (APT, APK,
...
`install.sh` at each feature root is generated from `features/install.sh` by `scripts/sync-src.py`. It is identical across all features. **Never write it manually.**
