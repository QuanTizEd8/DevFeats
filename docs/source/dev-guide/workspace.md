...
│   ├── logging.sh           ← Structured logging with log levels
│   ├── os.sh                ← OS/hardware detection
│   ├── ospkg.sh             ← Cross-distro package manager abstraction
│   ├── github.sh            ← GitHub API helpers
│   ├── net.sh               ← Network fetch, retry, checksum
│   ├── git.sh               ← Git operations
│   ├── shell.sh             ← Shell detection, startup file management
│   ├── users.sh             ← User/group management
│   ├── ver.sh               ← Version parsing and comparison
│   ├── install.sh           ← Installation state management, asset extraction
│   ├── *.sh                 ← Additional modules (oci.sh, json.sh, etc.)
...
| `docs/source/library/` | Auto-generated by `just build-docs` from `lib/*.sh` annotations |
