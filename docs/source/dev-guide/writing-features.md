...
2. Create `features/<feature-id>/metadata.yaml` following [feature-json.instructions.md](../../.github/instructions/feature-json.instructions.md) and validating against [`features/metadata.schema.json`](../../features/metadata.schema.json).
3. Create `features/<feature-id>/install.bash` (body only — no header;
  the header is generated by `scripts/sync-src.py`) following the structure
   described below.
4. Run `python3 scripts/sync-src.py` — this assembles `src/<feature-id>/install.bash`,
   generates `src/<feature-id>/install.sh` (bootstrap) and `src/<feature-id>/_lib/`.
5. Create `test/<feature-id>/scenarios.json` and at least one `<scenario>.sh`.
...
```
features/<feature-id>/
├── metadata.yaml               ← Metadata, options, _dependencies (source of truth)
├── install.bash                ← Body-only installer (header generated by sync)
└── files/                      ← Optional static files
```

After running `python3 scripts/sync-src.py`, the assembled feature is in `src/<feature-id>/` (git-ignored):
...
to `lib/` rather than keeping it inline, then run `python3 scripts/sync-src.py` to
propagate it. See [Sync](#sync).
...
Create a `_dependencies.base` key in `features/<feature>/metadata.yaml` when the installer needs tools
(e.g. `curl`, `ca-certificates`) to be present before any other work begins.
`sync-src.py` converts this to `src/<feature>/dependencies/base.yaml` automatically.
The format is a YAML ospkg manifest (see
[`features/install-os-pkg/NOTES.md`](../../features/install-os-pkg/NOTES.md) and [`manifest.schema.json`](../../features/install-os-pkg/manifest.schema.json) for the full format).
...
See [`features/install-os-pkg/NOTES.md`](../../features/install-os-pkg/NOTES.md) and the manifest schema for the complete
...
- [install-os-pkg — NOTES](../../features/install-os-pkg/NOTES.md) — usage; manifest schema: [`features/install-os-pkg/manifest.schema.json`](../../features/install-os-pkg/manifest.schema.json); see also [ospkg-manifests.instructions.md](../../.github/instructions/ospkg-manifests.instructions.md)
- [Repository structure](repo-structure.md) — how scripts/sync-src.py and features/bootstrap.sh work
