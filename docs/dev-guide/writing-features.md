...
2. Create `features/<feature-id>/install.bash` (body only — no header;
  the header is generated by `scripts/sync-lib.sh`) following the structure
   described below.
3. Run `bash scripts/sync-lib.sh` — this assembles `src/<feature-id>/install.bash`,
...
After running `bash scripts/sync-lib.sh`, the assembled feature is in `src/<feature-id>/` (git-ignored):
...
generated artefacts — they are created by `bash scripts/sync-lib.sh`. Never edit
them directly.
...
`scripts/sync-lib.sh`. It is identical in every feature. **Never write it manually.**
...
to `lib/` rather than keeping it inline, then run `bash scripts/sync-lib.sh` to
...
`lib/*` or `features/bootstrap.sh` will trigger `scripts/sync-lib.sh` automatically.
...
- [Repository structure](repo-structure.md) — how scripts/sync-lib.sh and features/bootstrap.sh work
