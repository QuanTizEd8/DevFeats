...
2. Create `features/<feature-id>/install.bash` (body only — no header;
  the header is generated by `scripts/sync-src.sh`) following the structure
   described below.
3. Run `bash scripts/sync-src.sh` — this assembles `src/<feature-id>/install.bash`,
...
After running `bash scripts/sync-src.sh`, the assembled feature is in `src/<feature-id>/` (git-ignored):
...
generated artefacts — they are created by `bash scripts/sync-src.sh`. Never edit
them directly.
...
`scripts/sync-src.sh`. It is identical in every feature. **Never write it manually.**
...
to `lib/` rather than keeping it inline, then run `bash scripts/sync-src.sh` to
...
`deps.py` converts this to `src/<feature>/dependencies/base.yaml` automatically.
...
`lib/*` or `features/bootstrap.sh` will trigger `scripts/sync-src.sh` automatically.
...
- [Repository structure](repo-structure.md) — how scripts/sync-src.sh and features/bootstrap.sh work
