...
> Edit `lib/` and run `bash scripts/sync-lib.sh` to propagate changes.
...
├── scripts/sync-lib.sh          ← Build tool: distributes lib/ and features/bootstrap.sh
...
scripts/sync-lib.sh           Distributes lib/ into every feature
```
...
| Generated output | `src/` | Assembled by scripts/sync-lib.sh — never edit directly |
| Tests | `test/` | Verification — never shipped |
| Shared library | `lib/` | Canonical bash helpers — never shipped directly |
| Build tooling | `scripts/sync-lib.sh`, `features/bootstrap.sh` | Assembles `src/` from `features/` + `lib/` |
...
`src/` is fully generated by `scripts/sync-lib.sh` and is git-ignored.
...
regenerated by `scripts/sync-lib.sh` (see below). Never edit files under `src/` directly.
...
`src/*/_lib/`. After editing, run `bash scripts/sync-lib.sh` to propagate
...
## Build tooling — `scripts/sync-lib.sh` and `features/bootstrap.sh`
...
`scripts/sync-lib.sh` automates the distribution of shared files:
...
CI runs `bash scripts/sync-lib.sh` as the first step of every workflow job to ensure
...
```
# Generated copies — synced by scripts/sync-lib.sh.
# Never commit these; CI runs scripts/sync-lib.sh after checkout.
src/**/_lib/
src/*/install.sh
```
...
All three workflows run `bash scripts/sync-lib.sh` as their first step before
