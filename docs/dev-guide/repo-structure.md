...
> Edit `lib/` and run `bash scripts/sync-src.sh` to propagate changes.
...
scripts/build-artifacts.sh    Assembles all dist/ artifacts
features/get.sh               Standalone single-feature installer source (version-stamped into dist/get.sh)
features/sysset.sh            Standalone manifest orchestrator source (version-stamped into dist/scripts/sysset.sh)
scripts/sync-src.sh           Distributes lib/ into every feature
```
...
| Generated output | `src/` | Assembled by scripts/sync-src.sh — never edit directly |
| Tests | `test/` | Verification — never shipped |
| Shared library | `lib/` | Canonical bash helpers — never shipped directly |
| Build tooling | `scripts/sync-src.sh`, `features/bootstrap.sh` | Assembles `src/` from `features/` + `lib/` |
...
`src/` is fully generated by `scripts/sync-src.sh` and is git-ignored.

### Feature source anatomy

```
features/<feature>/
├── metadata.yaml               ← Single source of truth: name, options, _dependencies
...
regenerated by `scripts/sync-src.sh` (see below). Never edit files under `src/` directly.
...
`src/*/_lib/`. After editing, run `bash scripts/sync-src.sh` to propagate
...
## Build tooling — `scripts/sync-src.sh` and `features/bootstrap.sh`
...
`scripts/sync-src.sh` automates the distribution of shared files:
...
CI runs `bash scripts/sync-src.sh` as the first step of every workflow job to ensure
...
|---|---|---|
| `sync-lib` | `lib/*`, `features/bootstrap.sh` | Regenerates `_lib/` copies and `install.sh` stubs |
...
```
# Generated copies — synced by scripts/sync-src.sh.
# Never commit these; CI runs scripts/sync-src.sh after checkout.
src/**/_lib/
src/*/install.sh
```
...
All three workflows run `bash scripts/sync-src.sh` as their first step before
