...
| Task | Command |
|------|---------|
| Sync auto-generated files | `bash scripts/sync-src.sh` |
| Verify auto-generated files are up to date | `bash scripts/sync-src.sh --check` |
| Format all shell files | `just format` |
| Check formatting (CI-style, no writes) | `just format-check` |
| Lint all shell files | `just lint` |
| Test one feature (scenarios + fail cases) | `bash test/run.sh feature <feature>` |
| Run lib/ unit tests (all) | `just test-unit` |
