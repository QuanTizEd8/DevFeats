...
The library is synced into each feature as `_lib/`, and scripts source them from `$_SELF_DIR/_lib/<module>.sh`. Each module uses a guard variable (`__<NAME>_LIB_LOADED`) to prevent double-sourcing and circular imports, so that modules can safely source each other.
