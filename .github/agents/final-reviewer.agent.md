...
1. **`features/<feature-name>/dev-notes/feature.md`** — installation research, citations, and API / options narrative
2. **`features/<feature-name>/dev-notes/implementation.md`** — implementation specification (if present)
3. **`features/<feature>/metadata.yaml`** — the YAML source for the JSON API
4. **`features/<feature>/install.bash`** — the script body (header generated by sync)

For each option, verify:
- Same name in reference doc sections, `metadata.yaml`, and script flag/env var.
