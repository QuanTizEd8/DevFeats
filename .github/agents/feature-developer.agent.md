...
- Follow [Development Workflow](/docs/source/dev-guide/workflow.md) and project conventions: explicit `return` statements, and `logging__*` helpers from `lib/logging.sh` (not raw `echo "…" >&2`) for user-visible stderr lines.
...
6. Create or update `features/<feature-name>/dev-notes/feature.md` with the API documentation from step 5 (per `.github/doc-templates/api.md`), alongside the research content. This file is the single developer reference for installation methods, citations, and the options API.
...
- Feature reference (installation research + API / options): `features/<feature-name>/dev-notes/feature.md`
- Implementation reference: `features/<feature-name>/dev-notes/implementation.md`
...
6. Write the installer script under `features/<feature-name>/install.bash` following all conventions:
   - **Body-only file** — no manual header; `just sync-src` prepends the generated header to `src/<feature-name>/install.bash`. See [`install.bash` reference](/docs/source/dev-guide/features/install.bash.md).
...
2. Run the following command in a background terminal without blocking, i.e. use `run_in_terminal` with `mode=async`:
   ...
   ```
   (Equivalent: `bash .dev/scripts/fetch/gha.sh --commit <commit-sha>`.)
...
- **Lefthook**: optional; hooks are **not** registered automatically in the dev container. Config lives in `.config/lefthook.yml`; run `lefthook install` once to enable. See [Quickstart](/docs/source/dev-guide/quickstart.md).
