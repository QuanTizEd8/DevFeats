...
- Follow [Development Workflow](/docs/source/dev-guide/workflow.md) and project conventions: explicit `return` statements, and `logging__*` helpers from `lib/logging.sh` for stderr messages.
...
1. If a Feature Reference document already exists at `features/<feature-name>/dev-notes/feature.md`, read it thoroughly and flag any concerns including missing information, possible inaccuracies, ambiguities, or areas that require further clarification.
...
6. Write the installer script under `features/<feature-name>/install.bash` following all conventions:
   - **Body-only** — no manual file header; sync generates the header in `src/`. See [`install.bash` reference](/docs/source/dev-guide/features/install.bash.md).
