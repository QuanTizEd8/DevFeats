...
- `dependencies/` – (optional) directory containing system package dependency manifests for the feature during installation and/or runtime. Manifests are consumed by `lib/ospkg.sh` to install dependencies; they support cross-platform dependency declaration and installation across different package managers and operating systems.
...
- `lib/`: The shared bash library, copied as-is to each feature during the build process. Functions are organized into modules by topic (e.g. `lib/ospkg.sh` for OS package management, `lib/file.sh` for file manipulation, etc.) and are documented with structured annotations that are rendered in the documentation website. Despite the separate files, the entire library is always loaded as a single unit (by sourcing `lib/__int__.sh` in `install.tmpl.bash`), since most modules have interdependencies and are not designed to be used in isolation. Since modules only contain function definitions and no top-level code, the order of sourcing does not matter.
